class CrisprConstructsController < ApplicationController
  ...
  def index
    @records = policy_scope(CrisprConstruct).order("lower(name)").page params[:page]
  end
  ...
  private
    ...
    def crispr_construct_params
      params.require(:crispr_construct).permit(:refseq_transcript, :ensembl_transcript, :addgene_id, :name, :target_id, :guide_sequence, :cloning_vector_id, :description, :vendor_id, :vendor_product_identifier, :construct_tag_ids => [], construct_tags_attributes: [:id,:_destroy])
    end
end
