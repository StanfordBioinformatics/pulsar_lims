...
class Biosample < ActiveRecord::Base
  ...
  has_many :batches, -> {uniq}, through: :batch_items
  ...
  has_many :chipseq_experiments, -> {unscope(:order).uniq}, through: :libraries
  has_many :atacseqs, -> {unscope(:order).uniq}, through: :libraries
  belongs_to :well
