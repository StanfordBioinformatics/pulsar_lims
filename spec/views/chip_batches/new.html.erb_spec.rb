require 'rails_helper'

RSpec.describe "chip_batches/new", type: :view do
  before(:each) do
    assign(:chip_batch, ChipBatch.new(
      :user => nil,
      :crosslinking_method => "MyString"
    ))
  end

  it "renders new chip_batch form" do
    render

    assert_select "form[action=?][method=?]", chip_batches_path, "post" do

      assert_select "input#chip_batch_user_id[name=?]", "chip_batch[user_id]"

      assert_select "input#chip_batch_crosslinking_method[name=?]", "chip_batch[crosslinking_method]"
    end
  end
end
